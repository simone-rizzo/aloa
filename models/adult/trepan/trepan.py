...
class Trepan:
    ...
    def build_tree(MIN_EXAMPLES_PER_NODE, MAX_NODES, trainX, oracle):
        ...
        labels = np.array(oracle.get_oracle_label(trainX))
...
class Oracle:
    ...
    def get_oracle_label(self, examples):
        '''
		Returns the label predicated by the oracle network for example
		'''
        res = self.network.predict(examples)
        return res
    ...
    def generate_constrained_examples_with_labels(self, constraints, num_examples):
        ...
        for i in range(0, num_examples):
            ...
            label = self.get_oracle_label(np.array([example]))
            examples[i, :] = example
