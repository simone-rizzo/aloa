...
def bernoulli_noise(bin_values, p):
    """
    Function that flips the bit with p probability.
...
class LabelOnly_attack(Attack):
    ...
    def neighborhood_noise_opti(self, vect, N, pd):
        """
    ...
    def robustness_score_label2(self, model, dataset, label, n, scaler=None):
        ...
        for row in tqdm(dataset):
            ...
            if y_true == y_predicted:
                perturbed_row = row.copy()
                variations = self.neighborhood_noise_opti(perturbed_row, n, percentage_deviation)
                output = model.predict(variations)
                score = np.mean(np.array(list(map(lambda x: 1 if x == y_true else 0, output))))
                scores.append(score)
            else:
    ...
    def carlini_binary_rand_robust(self, model, ds, ds_label, p, noise_samples=100, stddev=0.040, scaler=None):
        """
        Original LBLONLY paper perturbation function.
        :param model:
        :param ds:
        :param ds_label:
        :param p:
    ...
    def attack_workflow(self):
        ...
        if self.bb.explainer:
            if not self.bb.depth:
                f = open("../trepan_explainers/explainers/{}/{}/{}/attack_tr_{}{}.txt".format(self.db_name, self.bb.model_name,
                                                                                       self.model_type_folder, self.settings, "_lssdpt" if self.bb.lss_dpt else ""),
                         "w")
                f.write(report)
                f.close()

                f = open("../trepan_explainers/explainers/{}/{}/{}/attack_ts_{}{}.txt".format(self.db_name, self.bb.model_name,
                ...
                with open("../trepan_explainers/xai_tradeoff/{}/attack_tr.txt".format(self.bb.depth), "w") as f:
                    f.write(report)
                with open("../trepan_explainers/xai_tradeoff/{}/attack_ts.txt".format(self.bb.depth), "w") as f:
                    f.write(report2)
...
def worker_start(att_conf, NOISE_SAMPLES):
    """
    Worker function for parallel exectuion.
    :param att_conf:
    :param NOISE_SAMPLES:
    :return:
    """
    ...
    att = LabelOnly_attack(bb, NOISE_SAMPLES, db_name=ds_name, settings=att_conf['setting'])
    att.start_attack()

"""
This is the main script for execute the ALOA and LBLONLY attacks.
The ALOA (Agnostic Label Only attack) is a variation of the LBLONLY attack.
To use ALOA just set the setting of the attack to   [0, 0, 1]
To use LBLONLY original set the settings to         [0, 0, 0]
"""
...
if __name__ == "__main__":
    ...
    for model in ['nn', 'rf', 'dt']:
        for lss_dpt in [True, False]:
                ...
                att = Original_lblonly(bb, NOISE_SAMPLES, db_name=ds_name, settings=[0, 0, 0], write_files=False)
                att.start_attack()"""
