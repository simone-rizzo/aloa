...
for m in range(N_SHADOW_MODELS):
    ...
    pred_tr_proba = shadow.predict(tr)
    pred_tr_labels = np.argmax(pred_tr_proba, axis=1)
    ...
    df_final.to_csv("data/shadow_data_nn/shadow_{}_predicted_ds".format(m), index=False)

path = "data/shadow_data_nn"
...
for c in classes:
    ...
    # Saving of the model.
    filename = 'attacker_nn/attacker_class_{}.sav'.format(c)
...
trainset_predict_proba = bb.predict(train_set)
...
testset_predict_proba = bb.predict(test_set)
...
for c in classes:
    ...
    att_c = pickle.load(open("attacker_nn/attacker_class_{}.sav".format(c), 'rb'))
...
write_report = open("attacker_nn/jointed_test_measures.txt", "w")
