...
from trepan_explainers.trepan_generation import TrePanGenerator
import numpy as np
from tqdm import tqdm
import os


def trade_off_score(accuracy, depth, weight_accuracy=1.0, weight_depth=1.0):
    """
    This score balance the trade off between fidelity, understandability
     and privacy protection.
    :param accuracy: The accuracy value of the model.
...
def new_score_sorting(config_list, gs_scores):
    """
    Function that given the list of configuration and the accuracies.
    It orders the model configurations based on the trade_off_score.
    :param config_list:
    :param gs_scores:
    :return:
    """
    ...
    for i, conf in enumerate(config_list):
        ...
        sr = gs_scores[i]
        new_sr = trade_off_score(sr, depth, weight_accuracy=0.2, weight_depth=0.1)
...
def score_desired_fidelity(config_list, gs_scores, target_fidelity):
    """
    We select the model having accuracy greater or equal to 85
    than 85 and the minimum number of depth.
    :param config_list: list of dict, containing all the cofigurations parameters of the
    grid search.
    :param gs_scores: list of fidelity scores of the models.
    :target_fidelity: real number between 0-1 representing the desired fideliy.
    :return:
    """
    coupled = [(gs_scores[i], conf['max_depth'], conf) for i, conf in enumerate(config_list)]
    filtered = list(filter(lambda x: x[0] >= target_fidelity, coupled))
    filtered.sort(key=lambda x: x[0], reverse=False)
    print(filtered)
    return filtered[0][2]


def grid_search_params(train_set, train_label):
    tree_para = {'criterion': ['gini', 'entropy'], 'max_depth': [5, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80],
...
def train_explainer_regularized(ds_name, train_set, train_label, test_set, test_label, path_tuple, max_depth_l=[8, 9, 10, 11, 12, 13]):
    ...
    # Model selection with the new mitigation srategy.
    best_param = new_score_sorting(configurations_list, scores_list)

    # Model selection with preferred fidelity.
    # best_param = greater_than_85(configurations_list, scores_list)

    # Model selection with the best parameters basend on accuracy.
    # best_param = grid.best_params_
...
"""
Script for testing the Mitigation.
"""
if __name__ == "__main__":
    ...
    for i, ds_name in enumerate(ds_names):
        ...
        for bb in tqdm(bboxes):
            ...
            # Best param with grid search
            best_param = grid_search_params(tr_set, tr_label)
            # Best params
            train_explainer(ds_name, best_param, tr_set, tr_label, ts_set, ts_label,
